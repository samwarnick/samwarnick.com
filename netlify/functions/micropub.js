...
const micropubFn = async (event) => {
  ...
  if (res && res.filename) {
    if (action == "create") {
      const path = res.filename.replace(
...
let handler = middy(micropubFn);
handler = handler.before(async (request) => {
  const { headers } = request.event;
  if (headers["accept"] === "application/json" && request.event.body === "") {
    request.event.body = "{}";
  }
});
handler = handler
