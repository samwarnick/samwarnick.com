...
const readBooks = books.filter(
  ({ finishedReading, didNotFinish }) => !!finishedReading && !didNotFinish,
);
const years = readBooks.map(({ finishedReading }) =>
  new Date(finishedReading).getFullYear(),
);
...
const inProgressBooks = books.filter(
  ({ startedReading, finishedReading }) => !!startedReading && !finishedReading,
);
